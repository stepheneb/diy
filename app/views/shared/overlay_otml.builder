        ...
        if @learners.size > 0 || @userListURL
          ...
          if @userListURL
            ...
            xml.OTGroupListManager(hash) {
              xml.userList {
                @learners.each do |l|
                  xml.OTGroupMember("name" => l.user.name, "uuid" => l.user.uuid, "passwordHash" => l.user.password_hash, "dataURL" => "#{get_overlay_server_root}/#{l.runnable.id}/#{l.id}-data.otml", "isCurrentUser" => (l.user_id == @user.id), "sailUuid" => l.sds_workgroup_uuid) {
                    xml.userObject {
