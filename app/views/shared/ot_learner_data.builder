        ...
        if @useOverlays
          @learners.each do |learner|
            xml.OTUserDatabaseRef(
              ...
              "overlayURL" => "#{@overlay_root ? get_overlay_server_root(@overlay_root) : get_overlay_server_root}/#{learner.runnable.id}/#{learner.id}.otml#{@overlay_params ? "?#{@overlay_params}" : ""}"
            )
          end
        else
          @learners.each do |learner|
